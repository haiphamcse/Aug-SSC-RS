...
class SemanticKitti(torch.utils.data.Dataset):
    ...
    def get_filepaths(self,):
        ...
        for seq in self.sequences:
            ...
            self.filepaths['occupancy'] += sorted(glob(os.path.join(self.data_root.replace('sequences', 'sequences_msnet3d_sweep10'), seq, 'voxels', '*.pseudo')))
    ...
    def get_data(self, idx, flip_type):
        ...
        points_path = self.filepaths['occupancy'][idx].replace('voxels', '').replace('sequences_msnet3d_sweep10', 'unidepth/lidar/sequences').replace('pseudo', 'bin')
        ...
        data_collection['img'] = torch.from_numpy(img)
