...
def process_pc(f):
    ...
    path = f"data/dataset/unidepth/accumulated_lidar/sequences/{scene_id}/"
    ...
    if os.path.isfile(f_path):
        ...
        for i in targets:
            ...
            folder = "data/dataset/unidepth/lidar_color/sequences/{}/{:06d}.npy".format(scene_id, i)
        ...
        for i in range(len(voxels_)):
            ...
            if i != 0:
                mask = (vox[:, 2] < 4.4) & (vox[:, 0] < 30.0) & (vox[:, 0] > 3.0) 
            else:
                mask = (vox[:, 2] < 4.4)
            vox[:, :3] = new.T[:, :3]
...
if __name__ == "__main__":
    ...
    for scene_id in ids:
        ...
        folder = f"data/dataset/unidepth/lidar_color/sequences/{scene_id}" # change this to the file path on your machine
