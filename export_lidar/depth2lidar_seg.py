...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_dir', type=str, default='depth_dir/sequences/00')
    ...
    for fn in depths:
        ...
        image = np.load(args.img_dir + '/' + predix + '.npy')
        print(image.shape)
        lidar = project_disp_to_depth(calib, depth_map, args.max_high)
        # pad 1 in the indensity dimension
        lidar = np.concatenate([lidar, np.ones((lidar.shape[0], 1))], 1)
        # lidar = lidar.astype(np.float32)
        lidar_color = np.concatenate([lidar, image.reshape(-1,1)], 1)
        np.save('{}/{}.npy'.format(args.save_dir, predix), lidar_color)
